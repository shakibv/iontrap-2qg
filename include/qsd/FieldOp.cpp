...
namespace qsd {
    ...
    static thread_local int intSqrtSize = 0;
    static thread_local double *intSqrt = 0;
    static thread_local double *halfIntSqrt = 0;
...
#endif
        int i;
		std::complex<double> center = v.centerCoords();
        int vSize = v.size();
...
// `hc=HC' and `hc=NO_HC' are identical for Hermitian operators.
    {
        ...
        if (center != 0.0) {
//    error("Moving basis not implemented for AnnihilationOperator.");
			std::complex<double> previous, temp, vzero, vnth;
			std::complex<double> cstar = conj(center);
...
// `hc=HC' and `hc=NO_HC' are identical for Hermitian operators.
    {
        ...
        if (center != 0.0) {
            ...
            for (int i = 1; i < vSize - 1; i++) {
				...
				v[i] *= std::complex<double> (0, 1); // v[i].timesI();
                v[i] += pcent * temp;
                previous = temp;
            }
            v[0] = vzero;
        ...
        if (matrixSize < vSize) {
...
#endif
            matrixSize = vSize;
            vv = new std::complex<double>[vSize];
            matrix = new std::complex<double> *[vSize];
            for (k = 0; k < matrixSize; k++)
                matrix[k] = new std::complex<double>[vSize];
            ...
            for (n = 0; n < matrixSize; n++) {
                for (m = 0; m <= n; m++) {
                    ...
                    for (k = 0; k < n - m; k++)
                        s *= alpha * intSqrt[n - k] / (k + 1.0);
					std::complex<double> sum = s;
                    for (k = 1; k <= m; k++) {
