...
// the square roots of the integers more often than necessary.
    ...
    static thread_local double smallSize = 1.0e-8;
    static thread_local int intSqrtMax = 0;
    static thread_local double *intSqrtList = 0;
...
// assignment operator.
    ...
    void State::free() {
        ...
        if (data.size() > 0) {            // if memory is allocated...
#ifndef NON_GNU_DELETE
            // // delete[] data;            // recycle storage
			std::vector<std::complex<double>>().swap(data);
            if (nFreedoms > 1) {
                /*
				...
				*/
				...
				std::vector<int>().swap(nDims);
				std::vector<int>().swap(sizes);
				std::vector<int>().swap(nSkips);
				std::vector<int>().swap(partDims);
				std::vector<FreedomType>().swap(freedomTypes);
				std::vector<std::complex<double>>().swap(coords);
            }
    ...
    void State::copy(const State &a)    // make a copy of an existing state
    {
        ...
        if (nFreedoms != a.nFreedoms) {    // if no memory allocated, allocate it
            ...
            if (nFreedoms > 1) {
                nDims = std::vector<int>(nFreedoms);
                sizes = std::vector<int>(nFreedoms);
                nSkips = std::vector<int>(nFreedoms);
                partDims = std::vector<int>(nFreedoms);
                freedomTypes = std::vector<FreedomType>(nFreedoms);
                coords = std::vector<std::complex<double>>(nFreedoms);
            }
        }
        if (totalDim != a.totalDim) {    // if no memory allocated, allocate it
            totalDim = a.totalDim;
            data = std::vector<std::complex<double>>(totalDim);
        }
...
// might cause errors.
    ...
    {                             // multidimensional
...
#endif
        nFreedoms = n;
        nDims = std::vector<int>(n);            // allocate memory
        sizes = std::vector<int>(n);
        nSkips = std::vector<int>(n);
        partDims = std::vector<int>(n);
        freedomTypes = std::vector<FreedomType>(n);
        coords = std::vector<std::complex<double>>(n);
        totalDim = 1;
        for (i = 0; i < n; i++) {
#ifndef OPTIMIZE_QSD
        ...
        maxSize = totalDim;
        data = std::vector<std::complex<double>>(totalDim);        // allocate memory
        myPointer = data.data();
        mySize = totalDim;
        nSkip = 1;
        coord = 0;
    ...
    State::State(int n, std::complex<double> alpha, FreedomType form)     // Constructor for
    {                             // coherent state
#ifndef OPTIMIZE_QSD
        if (n < 2) error("Invalid dimension size in State constructor!");
        if (form != FIELD)
    ...
    State::State(int n, int nstate, std::complex<double> alpha, FreedomType form)
//
// Constructor for excited coherent state
    ...
    State::State(int n, std::complex<double> *elements, FreedomType form)
//
// Constructor for general 1 degree of freedom state
    ...
    void State::coherent(int n, std::complex<double> alpha)    // Create a coherent state
    {
#ifndef OPTIMIZE_QSD
        if (n < 2)
    ...
    void State::productState(int n, State *stateList)    // Create a product state
    {
        ...
        for (i = 0; i < totalDim; i++) {
            ...
            for (j = n - 1; j >= 0; j--) {
                ...
                if (element == 0.0) break;        // if zero, leave this inner loop
                k %= nSkips[j];
            }
	...
	std::complex<double> &State::operator[](int *indexList)
//
...
//
	...
	std::complex<double> State::elem(const int *indexList) const
//
...
//
	...
	std::complex<double> State::operator*(const State &a) const        // inner product
    {
#ifndef OPTIMIZE_QSD
        if (mySize != a.mySize)
    ...
    State &State::operator*=(const std::complex<double> &z)    // multiplication by Complex
    {
        int i, j;
    ...
    State &State::operator*=(ImaginaryUnit im)    // multiplication by i or -i
    {
        int i, j;
        if (im == IM)
            for (i = 0, j = 0; i < mySize; i++, j += nSkip) data[j] *= std::complex<double>(0,1); // data[j].timesI();
        else
            for (i = 0, j = 0; i < mySize; i++, j += nSkip) data[j] *= std::complex<double>(0, -1); // data[j].timesMinusI();
        return *this;
    }
...
//
    {
        ...
        s << "nFreedoms: " << a.nFreedoms << std::endl;
        s << "totalDim: " << a.totalDim << std::endl;
        s << "maxSize: " << a.maxSize << std::endl;
        s << "mySize: " << a.mySize << std::endl;
        s << "nSkip: " << a.nSkip << std::endl;
        s << "coord: " << a.coord << std::endl;
        s << "myType: " << a.myType << " FreedomType{0: ALL, 1: FIELD, 2: SPIN, 3: ATOM}" << std::endl;
        s << std::endl;
        if (a.nFreedoms > 1)
            for (i = 0; i < a.nFreedoms; i++) {
                s << "nDims[" << std::to_string(i) << "]: " << a.nDims[i] << std::endl;
                s << "sizes[" << std::to_string(i) << "]: " << a.sizes[i] << std::endl;
                s << "nSkips[" << std::to_string(i) << "]: " << a.nSkips[i] << std::endl;
                s << "partDims[" << std::to_string(i) << "]: " << a.partDims[i] << std::endl;
                s << "freedomTypes[" << std::to_string(i) << "]: " << a.freedomTypes[i]
                  << " FreedomType{0: ALL, 1: FIELD, 2: SPIN, 3: ATOM}" << std::endl;
                s << "coords[" << std::to_string(i) << "]: " << a.coords[i] << std::endl;
                s << std::endl;
            }
        for (i = 0; i < a.maxSize; i++) {
            s << "data[" << std::to_string(i) << "]: ";
            s << a.data[i] << std::endl;
        }
        return s;
    }
...
//
    {
        ...
        s >> a.myType;
        if (a.totalDim > 0) {
            a.data = std::vector<std::complex<double>>(a.totalDim);
            a.myPointer = a.data.data();
        }
        if (a.nFreedoms > 1) {
            a.nDims = std::vector<int>(a.nFreedoms);
            a.sizes = std::vector<int>(a.nFreedoms);
            a.nSkips = std::vector<int>(a.nFreedoms);
            a.partDims = std::vector<int>(a.nFreedoms);
            a.freedomTypes = std::vector<FreedomType>(a.nFreedoms);
            a.coords = std::vector<std::complex<double>>(a.nFreedoms);
            for (i = 0; i < a.nFreedoms; i++) {
                s >> a.nDims[i];
                s >> a.sizes[i];
                s >> a.nSkips[i];
...
// apply a PrimaryOperator to the State
    ...
    State operator*(const std::complex<double> &a, const State &b) {    // multiplication
        State result(b);
        return (result *= a);
    }

    State operator*(const State &b, const std::complex<double> &a) {    // multiplication
        State result(b);
        return (result *= a);
    }
	...
	std::complex<double> State::centerCoords() { return coord; }    // return center of coordinates

	std::complex<double> State::getCoords(int theFreedom) {
#ifndef OPTIMIZE_QSD
        if ((theFreedom < 0) || (theFreedom >= nFreedoms))
    ...
    void State::setCoords(std::complex<double> &alpha, int theFreedom)
//
// Reset the value of the phase space coordinate for a freedom.
    ...
    void State::displaceCoords(std::complex<double> &alpha, int theFreedom)
//
// Reset the value of the phase space coordinate for a freedom.
    {
#ifndef OPTIMIZE_QSD
...
// Move the center of coordinates to a new position in phase space.
    ...
    void State::moveToCoords(const std::complex<double> &alpha, int theFreedom, double shiftAccuracy) {
#ifndef OPTIMIZE_QSD
        if ((theFreedom < 0) || (theFreedom >= nFreedoms))
...
// Recenter the coordinates at the current mean position in phase space.
    ...
    void State::recenter(int theFreedom, double shiftAccuracy) {
        ...
        int i, j, k, l;
        State dummy = (*this);
		std::complex<double> magnitude, alpha;
        magnitude = (*this) * (*this);
        if (magnitude == 0.0) error("Can't recenter zero state!");
        if (nFreedoms == 1) {        // 1 freedom case
            ...
            for (i = 0; i < nSkip; i++)        // apply local annihilation op
                for (j = 0; j < maxSize; j += partDims[theFreedom]) {    // loop over other
                    dummy.myPointer = dummy.data.data() + i + j;        // indices
            ...
            mySize = maxSize;
            nSkip = 1;
            dummy.myPointer = dummy.data.data();
        }
...
// phase space.
    ...
    void State::moveCoords(const std::complex<double> &alpha, int theFreedom, double shiftAccuracy) {
#ifndef OPTIMIZE_QSD
        if ((theFreedom >= nFreedoms) || (theFreedom < 0))
            error("Illegal freedom passed to State::moveCoords!");
#endif
		std::complex<double> mag = (*this) * (*this);
        if (mag == 0.0) {                // move zero state
		...
		std::complex<double> delta = alpha / double(numshifts);        // according to accuracy
		std::complex<double> deltacc = conj(delta);            // requirement.
		std::complex<double> theCoord, phase = 0;
        if (nFreedoms == 1) {                // 1 freedom case
#ifndef OPTIMIZE_QSD
        ...
        phase *= std::complex<double>(0, 1); // phase.timesI();
...
// Shift the center of coordinates by a single infinitesimal step
    ...
    void State::moveStep(std::complex<double> &delta, std::complex<double> &deltacc) {
        int i, j;
		std::complex<double> psizero, psinth, temp, previous;
