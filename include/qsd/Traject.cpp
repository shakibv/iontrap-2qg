...
#define ERRCON 1.89e-4
    ...
    AdaptiveStep::AdaptiveStep(const State &psi, const Operator &theH,
                               std::vector<Operator> theL, double theEpsilon)
//
...
//
    ...
    Trajectory::Trajectory(const State &thePsiIni, double thedt,
                           IntegrationStep &theStepper, ComplexRandom *theRand, std::mt19937 *rndGen, double theT0)
//
...
//
    {
        ...
        odeint(psi, t, t + dt, epsilon, dtlast, 0.0, nok, nbad, rndm, rnd_gen);
        // generate deterministic evolution (and also stochastic)
    }
...
//
    {
        ...
        for (int i = 0; i < nX; i++) {        // compute expectation values for
            psi1 = psi;                // output...
            psi1 *= X[i];
			std::complex<double> expec = psi * psi1;
            psi1 *= X[i];
			std::complex<double> expec2 = psi * psi1 - expec * expec;
        ...
        for (int n = 1; n <= nOfSteps; n++) {
            for (int n1 = 0; n1 < dtsPerStep; n1++) {
                (*stepper)(psi, t, dt, dtlast, rndm, rnd_gen);
                t += dt;
...
// (which is the default), the final state is saved to the file `savedState'.
    {
        ...
        if (savedState) {
            ...
            if (!os) {
				...
				// // exit(1);
				throw "trajectory error!";
            }
            os << psi;
        }
    }

std::vector<TrajectoryResult> Trajectory::sumExp_vector(std::map<std::string, Operator> X, std::vector<std::string> fname, int dtsPerStep, int nOfSteps,
...
//
    {
        ...
        int nX = X.size();  // number of operators in X
		...
		while (k < nTrajectory) {
			...
			traj_result.timeout = false;
			traj_result.error = false;

            // // std::cout << "Computing trajectory " << k + 1  << endl;
            ++nTrajEff; // todo: needs to be moved to the correct line
            t = t0;
            psi = psi0;

            // Timestep = 0
            n = 0;
			for (auto pair : X) { // compute expectation values for output...

				std::string key = pair.first;
				Operator &observable = pair.second;
				...
				traj_result.observables[key].emplace_back(expec.real(), expec.imag(), expec2.real(), expec2.imag());
            }
            traj_result.state_v.emplace_back(psi);

            // Timestep > 0
            for (n = 1; n <= nOfSteps; n++) {

				if (traj_result.timeout) {
					k++;
					break;
				}

				if (traj_result.error) {
					break;
				}

				if (callback) {
					callback(traj_result, n - 1, t);
				}
                ...
                for (int n1 = 0; n1 < dtsPerStep; n1++) {
                    ...
                    if (std::chrono::duration_cast<std::chrono::seconds>(end - begin).count() > timeout) {
						...
						std::cout << "traj: " << k << ", " << "timed out" << std::endl;

						traj_result.timeout = true;
						result.push_back(traj_result);

                        break;
                    }

                    if (cfg.dynamic_cutoff) {
                        for (int deg : cfg.dynamic_degrees) {
                            psi.adjustCutoff(deg, cfg.cutoff_epsilon, cfg.cutoff_pad_size);
                        }
                    }

                    if (cfg.phonon_moving_basis) {
                        for (int deg : cfg.dynamic_degrees) {
                            psi.recenter(deg, cfg.shift_accuracy);
                        }
                    }
					...
					{
						std::cout << "traj: " << k << ", " << msg << std::endl;

						traj_result.error = true;
						result.push_back(traj_result);
				...
				if (cfg.log_progress)
                	std::cout << "traj: " << k << ", " << n * dtsPerStep << " of " << nOfSteps * dtsPerStep << std::endl;
			...
			for (n = 0; n <= nOfSteps; n++) {
				traj_result.t.push_back(t);
				t += dtsPerStep * dt;
			}

            // Saving the state to memory per trajectory
            traj_result.state = psi;
		...
		if (cfg.save_on_disk) {
			...
			for (i = 0; i < nTrajectory; i++) {
				for (j = 0; j < nX; j++) {

					std::string filename = fname[j] + "_worker_" + std::to_string(c_file + worker_id) + "_traj_" + std::to_string(i);
					fp_traj[filename] = fopen((cfg.log_dir + filename).c_str(), "w");

					fprintf(fp_traj[filename], "Trajectory %d \n", i + 1);
			...
			for (i = 0; i < nTrajectory; i++) {
				...
				for (n = 0; n <= nOfSteps; n++) {
					...
					j = 0;
					for (auto pair : result[i].observables) {

						std::string key = pair.first;
						std::vector<qsd::Expectation> &expec = pair.second;

						std::string filename = key + "_worker_" + std::to_string(c_file + worker_id) + "_traj_" + std::to_string(i);
...
//		if (h < 0.1*h) h *= 0.1;
    ...
    void AdaptiveStep::odeint(State &ystart, double t1, double t2, double eps, double &h1,
                              double hmin, int &nok, int &nbad, std::complex<double> *dxi)
//
...
//
    ...
    void AdaptiveJump::odeint(State &ystart, double t1, double t2, double eps, double &h1,
                              double hmin, int &nok, int &nbad, ComplexRandom *rndm, std::mt19937 *rnd_gen)
//
// Runge-Kutta driver with adaptive stepsize control  Integrate starting values
// ystart from t1 to t2 with accuracy eps.  h1 should be set as a guessed
// first stepsize, hmin as the minimum allowed stepsize (can be zero).  On
...
// they should give the same results
                // // prob = 1.0 - real(y * y); // default behaviour
				prob = real(y * y);
//      prob = total*hdid;
//

                // // if (real((*rndm)()) < prob) {	// make a jump? // default behaviour
				if (dis(*rnd_gen) >= prob) {
					std::cout << "jump!" << std::endl;
                    y = newsum;

					// // prob = real((*rndm)()); // default behaviour
					prob = dis(*rnd_gen);
