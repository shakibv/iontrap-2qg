...
//  ComplexFunction sinus = si; Complex y = sinus(2.0);'
    ...
    enum Command {
        OPERATOR, OPERATOR_HC, COMPLEX, REAL, IMAG, M_IMAG,
    ...
    public:
        ...
        Operator operator*(const std::complex<double> &) const;

        Operator operator*(double) const;

        Operator operator*(ImaginaryUnit) const;

        Operator &operator*=(const std::complex<double> &);

        Operator &operator*=(double);

        Operator &operator*=(ImaginaryUnit);

        friend Operator operator*(const std::complex<double> &, const Operator &);
    ...
    private:
        ...
        struct Stack {
            std::vector<Command> com;           // The command stack.
            std::vector<PrimaryOperator *> op;   // Stack of pointers to PrimaryOperator
            std::vector<std::complex<double>> c;             // Stack of complex scalars.
            std::vector<double> r;              // Stack of real scalars.
            std::vector<ComplexFunction> cf;    // Stack of complex-valued functions.
            std::vector<RealFunction> rf;       // Stack of real-valued functions.
        };
        struct StackPtr {
            int com;
            int op;
        ...
        Stack stack;
        double time;
        static thread_local int flag;    // Set by `Operator' constructors, tested by
