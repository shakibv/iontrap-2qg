...
std::complex<double> ComplexUniform::operator()()
{
  return std::complex<double>(pGenerator->asDouble(), 0.0 );
}
...
std::complex<double> ComplexNormal::operator()()
{
  for(;;) {
    ...
    if (w <= 1) {
      ...
      return M_SQRT1_2 * std::complex<double>(v1*y, v2*y);
...
std::complex<double> ComplexNormalTest::operator()()
...
RefinedComplexNormal::RefinedComplexNormal(RNG *g1,RNG *g2,int theOffset)
{
  ...
  offset = theOffset;
  cacheSize = 2*offset;
  cache = new std::complex<double>[cacheSize];
...
std::complex<double> RefinedComplexNormal::operator()()
{
  if (cacheCount < cacheSize)
    return cache[cacheCount++];
  else {
    std::complex<double>* xi1;
  	std::complex<double>* xi2;
    xi1 = new std::complex<double>[offset];
    xi2 = new std::complex<double>[offset];
    int i;
    for( i=0; i<offset; i++ ) {
      for(;;) {
	...
	if (w <= 1) {
	  double y = sqrt( (-2 * log(w)) / w);
	  std::complex<double> xi(v1*y, v2*y);
	  xi1[i] = xi;
	  break;
	}
      }
...
std::complex<double> ComplexQuadratic::operator()()
{
  double c[2];
  for( int i=0; i<2; i++ ) 
  for(;;) {
