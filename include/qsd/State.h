...
// of a particular type.
    ...
    enum ImaginaryUnit {
        IMAGINARY_UNIT, MINUS_IMAGINARY_UNIT
    };
...
// constructors and destructors
        ...
        State(int n, std::complex<double> *elements, FreedomType form = FIELD);
        ...
        // which has amplitude 1; form gives the FreedomType of the State
        // (default is FIELD).
        State(int n, std::complex<double> alpha, FreedomType form = FIELD);    // Coherent state
        // Produces a one degree-of-freedom State with n basis
        ...
        State(int n, int nstate, std::complex<double> alpha, FreedomType form = FIELD);
...
// public functions used by constructors and destructors
        ...
        void coherent(int n, std::complex<double> alpha);        // create a Coherent state
        void productState(int n, State *stateList);    // create a product state

// Member arithmetic operations

        inline std::complex<double> &operator[](int n) {            // subscript operator; gives
		...
		std::complex<double> elem(const int *) const;
		...
		std::complex<double> operator*(const State &) const;    // inner product
        State &operator*=(const std::complex<double> &);        // multiply by Complex scalar
...
// Friend arithmetic operations
        ...
        friend State operator*(const std::complex<double> &, const State &);

        // multiply a State by a Complex scalar:  z*psi
        friend State operator*(const State &, const std::complex<double> &);
...
// this, see the basis-changing member functions below.
		...
		std::complex<double> centerCoords();

        // return center of coordinates
		std::complex<double> getCoords(int = 0);

        // center of coordinates of nth freedom (default 0)
        void setCoords(std::complex<double> &, int= 0);

        // set the value of the coords for a freedom (default 0)
        void displaceCoords(std::complex<double> &, int= 0);
...
// For further details, see J. Phys. A 28, 5401-5413 (1995).
        ...
        void moveCoords(const std::complex<double> &displacement, int theFreedom = 0,
        ...
        void moveToCoords(const std::complex<double> &alpha, int theFreedom = 0,
                          double shiftAccuracy = 1e-4);
...
// ordinary array, but steps through memory by an ``skip'' larger than one.
		...
		std::complex<double> *myPointer;        // pointer to storage
...
// used for greater efficiency in 1 Freedom problems
		...
		std::vector<std::complex<double>> data;        // element storage; this points to an array
        // of complex numbers giving the amplitudes
        // of the basis states
		std::complex<double> coord;        // center of coordinates in phase space
...
//   nSkips[m] = n_0*n_1*...*n_(m-1).
        ...
        std::vector<int> nDims;            // number of dimensions (basis states)
        // for each freedom
        std::vector<int> sizes;            // number of dimensions (basis states)
        // actually used by each freedom
        std::vector<int> nSkips;            // index spacing in data field of ith freedom
        std::vector<int> partDims;        // subspace dim of first i freedoms
        std::vector<FreedomType> freedomTypes;    // types of degrees of freedom (FIELD, SPIN, etc.)
		std::vector<std::complex<double>> coords;        // centers of coordinates in phase space
...
// Private member functions
        ...
        void moveStep(std::complex<double> &, std::complex<double> &);    // shift coords by infinitesimal step
