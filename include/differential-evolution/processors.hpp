...
/*
 * Example objective_function_factory for the ion trap
 */
...
//    T_ptr of_ptr;
//
//    objective_function_factory (const std::string& config_path, std::ofstream* _log_file) {
//      _config_path = config_path;
//      __log_file = _log_file;
//
//      of.initialize(_config_path, __log_file);
//      of_ptr = boost::make_shared<T>(of);
//    }
//
//    /**
//     * Method implemented in derived classes that will create new
...
class processor_traits {
 public:
  ...
  static double run(T t, de::DVectorPtr vars, int genCount) { return t(vars, genCount); }
...
class processor : boost::noncopyable {
 ...
 public:
   ...
   *  			   objective function
   */
  processor(size_t index, T of, individual_queue& indQueue, processor_listener_ptr listener, int& genCount)
  ...
  void operator()() {
    ...
    try {
      for (individual_ptr ind = m_indQueue.pop(); ind; ind = m_indQueue.pop()) {
        ...
        double result = processor_traits<T>::run(m_of, ind->vars(), m_genCount);
 ...
 public:
  ...
  processors(size_t count, T of, processor_listener_ptr listener) {
    ...
    for (size_t n = 0; n < count; ++n) {
      processor_ptr processor(boost::make_shared<processor<T> >(n, of, boost::ref(m_indQueue), listener, m_genCount));
      m_processors.push_back(processors<T>::processor_ptr(processor));
    }
  }

  void set_genCount(int genCount) { m_genCount = genCount; }
