...
class population : public population_base {
 public:
  ...
  population(size_t popSize, size_t varCount, constraints_ptr constraints)
    ...
    /**
* constructs a population object and initializes its
* individuals by first using the guess as one individual
* and setting all variables of others to random values
* within the limits imposed by the corresponding constraints,
* then runs the objective function for each individual to
* calculate the associated cost.
*
...
*/
    population(size_t popSize, size_t varCount, constraints_ptr constraints,
               const std::vector<de::DVector>& guess_individuals) : population_base(popSize) {
        assert(popSize > 0);
        assert(varCount > 0);

        init(popSize, varCount);

		for (population::size_type i = 0; i < guess_individuals.size(); ++i) at(i)->init(guess_individuals[i]);
        for (population::size_type i = guess_individuals.size(); i < size(); ++i) at(i)->init(constraints);
    }
